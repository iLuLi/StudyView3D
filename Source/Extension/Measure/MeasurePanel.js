define([
    ...
    '../../Core/i18n',
    './SNAP'
], function(DockingPanel, ViewerPanelMixin, OptionDropDown, Logger, formatValueWithUnits, OptionCheckbox, i18n, SNAP) {
    'use strict';
    var SNAP_VERTEX = SNAP.SNAP_VERTEX;
    var SNAP_EDGE = SNAP.SNAP_EDGE;
    var SNAP_FACE = SNAP.SNAP_FACE;
    var SNAP_CIRCULARARC = SNAP.SNAP_CIRCULARARC;
    var SNAP_CURVEDEDGE = SNAP.SNAP_CURVEDEDGE;
    var SNAP_CURVEDFACE = SNAP.SNAP_CURVEDFACE;
    var SNAP_PRECISION = SNAP.SNAP_PRECISION;
    ...
    var MeasurePanel = function (measureTool, viewer, id, title, options) {
        ...
        xSpan1.textContent = i18n.translate("Delta");
        ...
        ySpan1.textContent = i18n.translate("Delta");
        ...
        zSpan1.textContent = i18n.translate("Delta");
        ...
        this.table.className = "adsk-lmv-tftable measure-table";
        ...
        this.unitList = new OptionDropDown("Unit type", this.tbody, unitNames, initialIndex);
        this.addEventListener(this.unitList, "change", function (e) {
            ...
            self.updatePanel();
            self.setupPrecision();
            Logger.track({ category: 'pref_changed', name: 'measure/units', value: toUnits });
        });

        this.precisionList = new OptionDropDown("Precision", this.tbody, [], -1);
        this.addEventListener(this.precisionList, "change", function (e) {
            ...
            self.updatePanel();
            Logger.track({ category: 'pref_changed', name: 'measure/precision', value: index });
        });
        this.setupPrecision();

        this.isolate = new OptionCheckbox("Isolate measurement", this.tbody, false);
        this.addEventListener(this.isolate, "change", function (e) {
            ...
            Logger.track({ category: 'pref_changed', name: 'measure/isolate', value: enable });
        });
        ...
        restart.textContent = i18n.translate("Restart");
    ...
    MeasurePanel.prototype = Object.create(DockingPanel.prototype);
    ViewerPanelMixin.call(MeasurePanel.prototype);
    ...
    MeasurePanel.prototype.updatePanel = function updatePanel() {
        ...
        this.initialDiv.textContent = formatValueWithUnits(0, null, 3, this.measureTool.getPrecision());
    ...
    MeasurePanel.prototype.showSelection1 = function showSelection1() {
        ...
        if (result.type === SNAP_VERTEX) { // Vertex
            ...
            this.selection1Result.textContent = i18n.translate("Vertex");
            ...
            this.selection1Result.textContent = i18n.translate("Edge") + " ~ " + this.measureTool.getEdgeLength(result.geometry);
            this.selection1Result.style.display = "inline-block";
        }
        else if (result.type === SNAP_FACE || result.type === SNAP_CURVEDFACE) { // Face

            this.selection1Result.setAttribute("data-i18n", "Face");
            this.selection1Result.textContent = i18n.translate("Face") + " ~ " + this.measureTool.getFaceArea(result.geometry);
            this.selection1Result.style.display = "inline-block";
        }
            ...
            if (result.geometry.radius)
                this.selection1Result.textContent = i18n.translate("Edge") + " ~ " + this.measureTool.getCircularArcRadius(result.geometry) + " (R)";
            else
                this.selection1Result.textContent = i18n.translate("Edge") + " ~ " + this.measureTool.getEdgeLength(result.geometry);
            this.selection1Result.style.display = "inline-block";
        }
    ...
    MeasurePanel.prototype.showSelection2 = function showSelection2() {
        ...
        if (result.type === SNAP_VERTEX) { // Vertex
            ...
            this.selection2Result.textContent = i18n.translate("Vertex");
            this.selection2Result.style.display = "inline-block";
        }
        else if (result.type === SNAP_EDGE || result.type === SNAP_CURVEDEDGE) { // Edge

            this.selection2Result.setAttribute("data-i18n", "Edge");
            this.selection2Result.textContent = i18n.translate("Edge") + " ~ " + this.measureTool.getEdgeLength(result.geometry);
            this.selection2Result.style.display = "inline-block";
        }
        else if (result.type === SNAP_FACE || result.type === SNAP_CURVEDFACE) { // Face

            this.selection2Result.setAttribute("data-i18n", "Face");
            this.selection2Result.textContent = i18n.translate("Face") + " ~ " + this.measureTool.getFaceArea(result.geometry);
            this.selection2Result.style.display = "inline-block";
        }
        else if (result.type === SNAP_CIRCULARARC) { // Circular Arc
            ...
            this.selection2Result.setAttribute("data-i18n", "Edge");
            if (result.geometry.radius)
                this.selection2Result.textContent = i18n.translate("Edge") + " ~ " + this.measureTool.getCircularArcRadius(result.geometry) + " (R)";
            else
                this.selection2Result.textContent = i18n.translate("Edge") + " ~ " + this.measureTool.getEdgeLength(result.geometry);
            this.selection2Result.style.display = "inline-block";
        }
    ...
    MeasurePanel.prototype.clearSelectionOne = function clearSelectionOne() {
        ...
        this.selection1Result.textContent = i18n.translate("Select Object");
    };
    ...
    MeasurePanel.prototype.clearSelectionTwo = function clearSelectionOne() {
        ...
        this.selection2Result.textContent = i18n.translate("Select Object");
    };
