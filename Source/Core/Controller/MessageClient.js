define([
    ...
    '../Logger',
    '../EventDispatcher'
], function(Global, Logger, EventDispatcher) {
    'use strict';
    ...
    function MessageClient(serverUrls, serverPath) {
        ...
        function getUserName() {
            if (Global.config.userName && Global.config.userName.length)
                return Global.config.userName;
    ...
    MessageClient.GetInstance = function (serverUrls, path) {
        ...
        if (!serverUrls)
            serverUrls = Global.EnvironmentConfigurations[Global.env].LMV.RTC;
