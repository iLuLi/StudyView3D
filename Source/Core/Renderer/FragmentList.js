define([
    ...
    var //visibility/highlight bitmask flags
        ...
        MESH_VISIBLE = Global.MESH_VISIBLE = 1,
        MESH_HIGHLIGHTED = Global.MESH_HIGHLIGHTED = 2,
        MESH_HIDE = Global.MESH_HIDE = 4,
        MESH_ISLINE = Global.MESH_ISLINE = 8,
        MESH_MOVED = Global.MESH_MOVED = 16,   // indicates if an animation matrix is set
        MESH_TRAVERSED = Global.MESH_TRAVERSED = 0x20, // only used for paging: drawn fragments are tagged and then skipped by forEach() until the flag is being reset (e.g. on scene/camera changes)
        MESH_RENDERFLAG = Global.MESH_RENDERFLAG = 0x80;
    /**
     * @constructor
    ...
    function FragmentList(model, loader) {
        ...
        if (this.isFixedSize) {
            ...
            this.useThreeMesh = !Global.memoryOptimizedLoading;
        } else {
            this.boxes = null;
