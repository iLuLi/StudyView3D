define([
    ...
    './Global',
    './VBIntersector'
], function(Logger, Private_Global, VBIntersector) {
    'use strict';
    var _tmpBox;
    ...
    if (Private_Global.pageOutGeometryEnabled) {
        ...
        RenderBatch.prototype.forEach = function (callback, drawMode, includeEmpty) {
            ...
            var pageOutGeometryEnabled = (frags.getCount() > Private_Global.FRAGS_PERSISTENT_MAX_COUNT);
            ...
            for (var i = this.start, iEnd = this.lastItem; i < iEnd; i++) {
                ...
                if (pageOutGeometryEnabled) {
                    ...
                    if ((frags.isFlagSet(idx, Private_Global.MESH_TRAVERSED)) && (drawMode == Private_Global.MESH_RENDERFLAG)) {
                        continue;
                    }
                    ...
                    if (!includeEmpty && (drawMode && frags.isFlagSet(idx, drawMode))) {
                        ...
                        if (!m.geometry) {
                            ...
                            if (drawMode == Private_Global.MESH_RENDERFLAG)
                                frags.setFlagFragment(idx, Private_Global.MESH_TRAVERSED, true);

                            // For fragments that may be paged out, check if this fragment was the
                            // last one 
                            if (idx > Private_Global.FRAGS_PERSISTENT_COUNT) {
    ...
    function evalVisbility(drawMode, vizflags, idx) {
        ...
        switch (drawMode) {
            ...
            case Private_Global.RENDER_HIDDEN:
                v = !(vfin & Private_Global.MESH_VISIBLE); //visible (bit 0 on)
                break;
            case Private_Global.RENDER_HIGHLIGHTED:
                v = (vfin & Private_Global.MESH_HIGHLIGHTED); //highlighted (bit 1 on)
                break;
            default:
                v = ((vfin & (Private_Global.MESH_VISIBLE | Private_Global.MESH_HIGHLIGHTED | Private_Global.MESH_HIDE)) == 1); //visible but not highlighted, and not a hidden line (bit 0 on, bit 1 off, bit 2 off)
                break;
        }

        //Store evaluated visibility into bit 7 of the vizflags
        //to use for immediate rendering
        vizflags[idx] = vfin | (v ? Private_Global.MESH_RENDERFLAG : 0);
    ...
    //TODO: This might be better done using inheritance from RenderBatch
    if (Private_Global.pageOutGeometryEnabled || Private_Global.onDemandLoading) {
        ...
        RenderBatch.prototype.applyVisibility = function () {
            ...
            function applyVisCB(m, idx) {
                ...
                if (culled) {
                    ...
                    vizflags[idx] = vizflags[idx] & ~Private_Global.MESH_RENDERFLAG;
            ...
            return function (drawModeIn, frustumIn, fragIdCbIn) {
                ...
                var containment = frustum.intersectsBox((drawMode === Private_Global.RENDER_HIDDEN) ? this.boundingBoxHidden : this.boundingBox);
                if (containment === Private_Global.OUTSIDE)
                    return allHidden; //nothing to draw

                vizflags = this.frags.vizflags;
                frags = this.frags;
                checkCull = (containment !== Private_Global.CONTAINS);
