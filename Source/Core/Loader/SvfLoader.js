define([
    ...
    var SvfLoader = function (parent) {
        ...
        this.logger = Logger;
        this.loadTime = 0;
        this.domainParam = (Global.auth && !DeviceType.isNodeJS) ? ("domain=" + encodeURIComponent(window.location.origin)) : "";
            ...
            if (ew.data && ew.data.manifest) {
                ...
                if (!isGltf) {
                    ...
                    if (numGeomPacks == 0) {
                        ...
                        if (svf.fragments.length > Global.FRAGS_PERSISTENT_MAX_COUNT) {
                            scope.onDemandLoading = Global.onDemandLoading;
                            scope.cullGeometryOnLoading = Global.cullGeometryOnLoading;
                        }
                        else {
    ...
    SvfLoader.prototype.processReceivedMesh = function (mdata) {
        ...
        if (this.cullGeometryOnLoading) {
            ...
            for (var i = 0; i < fragIndexes.length; ++i) {
                ...
                if (fragIndexes[i] < Global.FRAGS_PERSISTENT_COUNT) {
                    culled = false;
                    break;
                }
    ...
    SvfLoader.prototype.onModelRootLoadDone = function (svf) {
        ...
        if (svf.proteinMaterials && Global.PROTEIN_ROOT && Global.PRISM_ROOT) {
    ...
    SvfLoader.prototype.onGeomLoadDone = function () {
        ...
        if (!this.svf.proteinMaterials || !Global.PROTEIN_ROOT || !Global.PRISM_ROOT) {
