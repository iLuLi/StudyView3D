define([
    ...
    function GeometryList(model) { // model: instanceof RenderModel (required)
        ...
        if (this.svf.packFileTotalSize) {
            ...
            if (estimatedGPUMem <= Privite_Global.GPU_MEMORY_LIMIT &&
                numObjects < Privite_Global.GPU_OBJECT_LIMIT)
    ...
    GeometryList.prototype.addGeometry = function (geometry, numInstances, svfid) {
        ...
        var GPU_MEMORY_LOW = Privite_Global.GPU_MEMORY_LIMIT;
        var GPU_MEMORY_HIGH = 2 * GPU_MEMORY_LOW;
        var GPU_MESH_MAX = Privite_Global.GPU_OBJECT_LIMIT;
        if (this.isf2d)
        ...
        if (Privite_Global.memoryOptimizedLoading && !this.is2d) {
